...
export default function HomeScreen() {
  ...
  const fetchTankData = async () => {
    try {
      if (cliente) {
        const response = await axios.get('http://tuizak.com/api/tinaco', {
  ...
  const handleRequestFill = async () => {
    try {
      if (cliente) {
        ...
        const response = await axios.post('http://tuizak.com/api/mensajes', {
        ...
        await Promise.all(administrativos.map(admin => {
          return axios.post('http://tuizak.com/api/mensajes', {
            id_cliente: cliente.id,
      ...
      <ImageBackground source={require('../assets/images/background.png')} style={styles.background}>
        ...
        <View style={styles.main}>
          ...
          <View style={styles.tank}>
            <Text style={styles.levelText}>Nivel: {tankData?.Nivel || '83'}%</Text>
            <View style={styles.levelBarContainer}>
              <View style={[styles.levelBar, { width: `${tankData?.Nivel || 83}%` }]} />
            </View>
          </View>
        </View>
